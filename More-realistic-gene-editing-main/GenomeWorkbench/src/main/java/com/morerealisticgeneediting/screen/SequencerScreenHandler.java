...
public class SequencerScreenHandler extends ScreenHandler {
    ...
    @Nullable
    private final SequencerBlockEntity blockEntity;

    // Client-side constructor
    public SequencerScreenHandler(int syncId, PlayerInventory playerInventory) {
        this(syncId, playerInventory, new SimpleInventory(2), new ArrayPropertyDelegate(1)); // Delegate tracks state enum
    }

    // Server-side constructor
    public SequencerScreenHandler(int syncId, PlayerInventory playerInventory, Inventory inventory, PropertyDelegate delegate) {
        ...
        this.blockEntity = (inventory instanceof SequencerBlockEntity) ? (SequencerBlockEntity) inventory : null;
    ...
    public boolean onButtonClick(PlayerEntity player, int id) {
        ...
        // Assuming button ID 0 is the 'start' button.
        if (id == 0) {
            // All security and logic is handled by the block entity itself.
            blockEntity.startSequencing(player);
            return true;
        }
